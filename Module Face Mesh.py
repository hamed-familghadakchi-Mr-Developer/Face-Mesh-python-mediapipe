...
class FaceMeshDetector():
    def __init__(self, staticMode= False, maxFaces = 2, minDetectionCon=0.5,minTrackingCon=0.5):
        ...
        self.faceMesh = self.mpFaceMesh.FaceMesh(static_image_mode = self.staticMode,max_num_faces = self.maxFaces,
                                                 min_detection_confidence = self.minDetectionCon,min_tracking_confidence = self.minTrackingCon)


        self.drawSpec = self.mpDraw.DrawingSpec(color=(0, 128, 0), thickness=2, circle_radius=1)
    ...
    def findFaceMesh(self,img ,draw = True):
        ...
        if self.results.multi_face_landmarks:
            for faceLms in self.results.multi_face_landmarks:
                ...
                # to have acess to the location of each land mark
                    face = []
